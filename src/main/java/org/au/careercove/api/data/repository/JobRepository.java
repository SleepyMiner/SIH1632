...
public interface JobRepository extends PagingAndSortingRepository<Job, String> {
    ...
    @Transactional
    @Query("SELECT j FROM Job j WHERE (:title IS NULL OR j.title = :title) AND (:organizationName IS NULL OR j.organizationName = :organizationName) AND (:workMode IS NULL OR j.workMode = :workMode) AND (:sector IS NULL OR j.sector = :sector)")
